<#
Last modified: 2023-08-12 20:38:50
...
($time + (Get-Content '..\dist\js\ax.min.js')) | Set-Content -Encoding UTF8 '..\dist\js\ax.min.js'
($time + (Get-Content '..\dist\css\ax.min.css')) | Set-Content -Encoding UTF8 '..\dist\css\ax.min.css'
#Step 9: Exit the powershell window automatically after execution
#步骤9：执行完毕自动退出powershell窗口
#Stop-Process -Id $PID;